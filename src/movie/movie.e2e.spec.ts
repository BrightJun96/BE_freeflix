...
describe("MovieController (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    const movieUserLikeRepository =
      dataSource.getRepository(MovieUserLike);
  ...
  describe("[GET /movie/{id}]", () => {
    it("should get movie by id", async () => {
      ...
      const { body, statusCode } = await await request(
        app.getHttpServer(),
      )
        .get(`/movie/${movieId}`)
        .set("authorization", `Bearer ${token}`);
  ...
  describe("[POST /movie]", () => {
    it("should create movie", async () => {
      const {
        ...
        .post(`/file-upload`)
        .set("authorization", `Bearer ${token}`)
        .attach("file", Buffer.from("test"), "movie.mp4")
        .expect(201);
      ...
      expect(body.genres.map((x) => x.id)).toEqual(
        dto.genreIds,
      );
  ...
  describe("[POST /movie/{id}/like]", () => {
    it("should like a movie", async () => {
      ...
      console.log("like first movieId : ", movieId);
    ...
    it("should cancel like a movie", async () => {
      const movieId = movies[1].id;
      console.log("like cancel movieId : ", movieId);
  ...
  describe("[POST /movie/{id}/dislike]", () => {
    it("should dislike a movie", async () => {
      ...
      expect(body).toBeDefined();
      expect(body.isDislike).toBe(true);
    });
    ...
    it("should cancel dislike a movie", async () => {
      ...
      expect(body).toBeDefined();
      expect(body.isDislike).toBe(false);
    });
  });
});
